...
extension Network {
    ...
    open class func post(url:String, parameter:Dictionary<String, Any>, success: @escaping ((Data) -> Swift.Void), fail: @escaping ((Error) -> Swift.Void)) -> URLSessionTask? {
        guard let request = Network.instance.request(httpMethod: self.httpMethod.post.rawValue, url: url, parameter: (parameter as NSDictionary).jsonData) else {
    ...
    open class func get(url:String, success:@escaping ((Data) -> Swift.Void), fail:@escaping ((Error) -> Swift.Void)) -> URLSessionTask? {
        guard let request = Network.instance.request(httpMethod: self.httpMethod.get.rawValue, url: url, parameter: nil) else {
